...
function admin_head_addition() {	
	...
	wp_enqueue_style('admin-css-file', $admin_stylesheet_url);
	...
	<?php foreach ($options as $value) { ?>
		...
		<table  id="table_<?php echo $value['id']; ?>" class="table_<?php echo $value['className']; ?>">
			...
			<td  class="rt_panel_td_right">
			 <input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo htmlentities(get_option( $value['id'] )); } else { echo htmlentities($value['std']); } ?>" size="40" class="input_<?php echo $value['className']; ?>" />
			</td>
		</tr> 
		</table>
		<!--/ text box -->
		...
		<table >
			...
			<td class="rt_panel_td_right">
					...
					//once secilmisleri listeleyelim sirasiyla
								$secilen_sayfalar = explode(',', get_option($value['id']) );
					...
					foreach ($value['options'] as $op_val=>$option) { ?>
							...
							if (get_option($value['id']) != "" ) {			
								$bol=explode(',',get_option($value['id']));
								foreach ($bol as $ddd){
										if ($ddd == $op_val && $ddd != 0) {  
											$atla=1;
										}
		...
		<table >
			...
			<td class="rt_panel_td_right">
					<select style="width:240px;height:200px;" size="2"  multiple name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" title="Please Select">
					<?php foreach ($value['options'] as $op_val=>$option) { ?><option <?php  if (get_option($value['id']) != "" ) {$bol=explode(',',get_option($value['id']));foreach ($bol as $ddd){if ($ddd == $op_val && $ddd != 0) { echo ' selected="selected"';}}}?>  value="<?php echo $op_val; ?>"><?php echo $option; ?></option><?php } ?></select>
			</td>
		</tr> 
		</table>
		<!--/ select -->
...
function rt_ud_sidebars_list(){
	...
	echo "<fieldset class=\"rt_fieldset\"><lagend>Manually Created Sidebars</lagend>";
		...
		$rt_ud_sidebars= explode(';',get_option('rttheme_ud_sidebars'));
		for ($i=0;$i<count($rt_ud_sidebars)-1;$i++){
			$parameters = explode(',',$rt_ud_sidebars[$i]);
			$parts= array(
				"page" => "Pages",
				"post" => "Posts",
				"cat" => "Categories",
