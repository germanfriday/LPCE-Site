...
class RT_THEME_SLIDER_WIDGET extends WP_Widget {
    function RT_THEME_SLIDER_WIDGET() {
        ...
        parent::__construct( 'css-rt-theme-slider-widget', 'RT-THEME SLIDER WIDGET', $widget_ops );
    }
    ...
    function widget($args, $instance) {		
	   ...
	   if ($id=="Home Page Slider"){$slider=1;}
	   $open_page = "";
        ?>
		...
		<?php if ($slider):?>
			...
			<div class="slide">
				...
				<?php if($slider_background_image):?>
					...
					<?php if(get_option('rttheme_slider_auto_disable_box')):?>
					   
					   <?php
					   // Resize Slider Image
					   $imgURL  = ($slider_background_image);
					   $crop 	= true;
					   if($imgURL) $image_thumb = vt_resize( '', $imgURL, 940, 0, ''.$crop.'' );
					   ?>
					   <img src="<?php echo $image_thumb["url"];?>" alt="" class="image" />
						
					<?php else:?>
						<img src="<?php echo $slider_background_image;?>" class="image" alt="" />
					<?php endif;?>						    
	...
	global $post;
		/* link to */
		$linkto = isset( $instance['linkto'] ) ? esc_attr($instance['linkto']) : "";
		...
		$target_page= isset( $instance['target_page'] ) ? esc_attr($instance['target_page']) : "";
		
		/* slide backgroun image */
		$slider_background_image=isset( $instance['slider_background_image'] ) ? esc_attr($instance['slider_background_image']) : ""; 
		...
		$rt_page_id = isset( $instance['rt_page_id'] ) ? esc_attr($instance['rt_page_id']) : "";
		
		/* posts */
		$rt_post_id = isset( $instance['rt_post_id'] ) ? esc_attr($instance['rt_post_id']) : "";
	...
	<div class="manual_rest show_true">
		...
		<p><label for="<?php echo $this->get_field_id('subtitle'); ?>"><?php _e('Second Title:'); ?> <input class="widefat" id="<?php echo $this->get_field_id('subtitle'); ?>" name="<?php echo $this->get_field_name('subtitle'); ?>" type="text" value="<?php echo $subtitle; ?>" /></label></p>				
